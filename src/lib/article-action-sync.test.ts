...
import {
  ...
  computeArticlesCost,
  type SyncArticle,
} from "./article-action-sync"

const article = (status: string, price?: number | null): SyncArticle => ({
  status,
  price,
})
...
describe("computeActionSyncIfChanged", () => {
  it("reports no change when cost and status already match", () => {
    ...
    const currentAction = { cost: 10, status: ACTION_STATUS.EN_COURS as string }
  ...
  it("reports a change when the status drifted", () => {
    ...
    const currentAction = { cost: 15, status: ACTION_STATUS.EN_COURS as string }
    expect(computeActionSyncIfChanged(currentAction, articles)).toEqual({
      changed: true,
      cost: 15,
      status: ACTION_STATUS.TERMINEE,
